...
:::
:::Card pink Historical Note!
Faraday's Law is one equation that captures *two* **different** phenomena: The generation of an EMF when the magnetic field inside of a coil changes *and also* that altering or moving a coil in the presence of a magnetic field generates an EMF. It  may seem silly in hindsight, but this strange "relativity" was one of the curiosities that led Einstein to develop special relativity[fn]https://en.wikipedia.org/wiki/Faraday%27s_law_of_induction#Faraday's_law_and_relativity[/fn]
:::
...
$$
\vec F_{tot} = - m g \hat z  - 0.044 \Big (\frac{\mu_0^2 \sigma s}{r^4} \Big)  b^2 v \hat v
$$
where $m$ is the magnet's mass, $g = 9.81$ m s$^{-2}$, $\sigma$ is the electrical conductivity of aluminum, $s$ is the thickness of your tube, $r$ is the inner radius, $v$ is the speed of the magnet, $\mu_0$ the vacuum permeability, and $b$ is the magnetic moment.
