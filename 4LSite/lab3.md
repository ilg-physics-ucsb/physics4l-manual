...
We start by combining  [Eq](#Eq-cap1) and [Eq](#Eq-ohm) through equating $I(t)$. We find that the voltages across the capacitor and resistor, at any point in time, are related by
:::Equation vcvr
$$
...
We can now apply the constraint given in [Eq](#Eq-constraint) to eliminate $V_C(t)$ from the equation. Substituting $V_C(t) = V_0 - V_R(t)$, and noting that $V_0$ is a constant, and therefore it's derivative is 0:
...
The left hand side of this equation is a common integral left as an exercise. The right hand side contains only constants in the integrand. The result is therefore:
$$
 \ln (V_R(t)/V_0) = -\frac{t}{RC}
$$

Exponentiating both sides gives us our final results for the voltage across the resistor over time:
...
Imagine you've charged your capacitor to some voltage $V_{max}$ and then turned off the power source.
1. After the power source has been switched off, what is the constraint on the voltages of the resistor and capacitor?
2. Combining  [Eq](#Eq-cap1) and [Eq](#Eq-ohm), like before, we get [Eq](#Eq-constraint)
$$
