...
Resistors -- as implied by their name -- will resist charge flow. They convert some of the kinetic energy of electrons into *heat*. While all materials [fn]except those that obtain a *superconducting* state at very low temperatures[/fn] have resistance, resistors are specially constructed circuit components with known resistance values that are much higher than most metals, and have *Ohmic* behavior. 
...
### Capacitor Dynamics
First, let's understand how capacitors in a circuit behave. Using the definition of current in [Eq](#Eq-currentcharge), we have
...
Here we see that the *change in the voltage across the capacitor* at any point in time is equal to the current divided by the capacitance. This doesn't mean that a capacitor can charge forever: Recall that *the current itself* is generated by a potential difference across a wire. As the capacitor acquires charge, the potential difference between the capacitor and source will monotonically decrease until *no current flows*, e.g. $\frac{dV}{dt} = \frac{0}{C}=0$ 



If we remove the power source (e.g. turn off the power block), then the potential difference between the capacitor and ground reverses, so that charges will flow *out of the capacitor* following the same equation[fn]Note that since the direction of the potential difference changes, $I(t)$ will pick up a negative sign![/fn].

### The Basic RC Circuit

In a circuit with a highly conductive wire these processes will happen very quickly since charges are free to accelerate along the potential line with minimal resistance. In order to see this phenomena on human timescales, we must add a resistor to our circuit.
...
As charge flows through our resistor, the potential difference moves to the capacitive element. Since the total potential difference of the circuit measured across the battery is $V_0$, we know that $V_C +V_R = V_0$. Combining  [Eq](#Eq-cap1) and [Eq](#Eq-ohm) by equating $I(t)$, we find that the Voltage across the resistor is given by: 
...
This is a first order differential equation that may be integrated to yield:

:::Equation
$$
\begin{aligned}
C \int_{V_0}^{V_R(t)} \frac{dV_R}{V_R} &= -\int_0^{t} \frac{1}{R} dt \\\\
 \ln (V_R(t)/V_0) &= - \frac{t}{RC} 
\end{aligned}
$$
:::

Rearranging this equation, gives us our final results for the voltage across the resistor over time:
...
This means that during the **charging cycle**, the time that the voltage across your resistor or capacitor reaches $37$% and $63$% of the maximum, respectively, is a measure of $\tau$. 

For technical reasons related to the multimeter, we will be measuring the voltage across the capacitor in this experiment. Since the capacitor takes a long time to charge, we will be interested in studying the **discharging cycle**. This is easier to work with since you know the voltage across the capacitor the moment you turn off the power, and you don't have to guess at what the maximum voltage is! You will work out the equation needed in [Qu](#Qu-check)
...
4. The equations given describe the behavior of a *charging* capacitor. In the following experiments, we will want to work with a discharging circuit since we will know the initial voltage across a charged capacitor. Deduce the equation for $V_C(t)$ on a *discharging* capacitor, where at $t=0$ $V_C= V_0$ and the power source is shut off. What is the value of $V_C$ when $t=\tau$?
:::
...
Even though the resistors and capacitors are labeled in the electronics kit, it's useful to learn how the resistance is color coded. It is recommended to double check the 5-band code to make sure the resistors you are using is correct for the lab, especially if you have removed them from their packaging already.
...
2. Using the uncertainty bounds of you resistor and capacitor calculate the error bound $\delta_\tau$  of this measurement. 
...
3. Write your final result as $(\tau \pm \delta_\tau)$. The bounds you have calculated on $\tau$ are your expected error bounds. Due to the tolerances of your components, you would expect that even if an experiment was executed perfectly, your final result would typically end up somewhere in this interval due to **uncertainty** in your components' actual values.
::::::
...
5. Turn on your circuit and fully charge your capacitor. You will know the capacitor is fully charged when the reading on your multimeter stops changing.
6. Once your reading stops changing, record the value you observe.
...
::: Question
What are the values calculated from charge and discharge curves? Are they close? Is it sufficient to measure $\tau$ from only one of the curves? Explain your reasoning.
:::

::: Question
1. What is the average experimental value of $\tau$ measured? Call this your $\tau_{measured}$

2. Calculate the discrepancy with the theoretical value calculated in [Qu](#Qu-theorytau). The discrepancy is defined by the quantity 
$$
\text{discrepancy} = |\tau_{theoretical} - \tau_{measured} |
$$

3. Is the discrepancy smaller than your error bounds? If not, propose some reasons for this: describe possible sources of systematic error, *e.g.*, external factors like humidity or poor contact, that might lead to shorter discharge times, dying batteries, inaccurate timing.
